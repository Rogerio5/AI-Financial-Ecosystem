    ...
    def discover_in(
        ...
        *,
        ...
        try:
            ...
            # This also happens when running in a vanilla Python REPL or when code is wrapped into
            # an exec call.
            source = ""

        if not source:
            if inspect.isfunction(obj) or inspect.iscode(obj):
                logger.warning("Failed to get function source code. \
                    Automated dependency discovery is not supported in Python shell \
                        or in code wrapped into an exec call.")
            return deps
        ...
        if not inspect.ismodule(obj) and module is not None:
            if not hasattr(module, '__file__') or module.__file__ is None:
                logger.debug("Detected interactive Python environment. Using session history for dependency discovery.")
                fallback_source = cls._try_get_ipython_executed_code()
            else:
                try:
        ...
        if fallback_source:
            fallback_source = textwrap.dedent(fallback_source)

        deps._scan_source(source, package, fallback_source, packages_distributions)
        return deps

    @staticmethod
    def _try_get_ipython_executed_code() -> str | None:
        try:
        ...
        ipython = get_ipython()
        if ipython is None:
            logger.debug("There's no currently running IPython session.")
            return None

        in_list = ipython.user_ns.get("In", [])
        ...
        for cell in in_list:
            ...
            try:
                ...
                logger.debug("Skipping cell with invalid Python syntax: %s", cell[:50])
                continue
