...
class ArrowTableToRowsConversion:
    ...
    def _create_converter(
        ...
        # BEGIN-EDGE
        # This is kept in sync with the same conversion logic in pyspark/sql/pandas/types.py,
        # but with an added None check.
        elif isinstance(dataType, VariantType) and IS_ENGINE_PYSPARK:
        ...
        # END-EDGE
            ...
            def convert_geography(value: Any) -> Any:
                if value is None:
                    ...
                    # BEGIN-EDGE
                    # Keep in sync with the same conversion logic in pyspark/sql/pandas/types.py
                    if IS_ENGINE_PYSPARK:
...
class ArrowArrayConversion:
    ...
    def convert_array(
        ...
        if check_type(pa_type):
            ...
            return pa.StructArray.from_arrays(
                ...
                # BEGIN-EDGE
                # ES-1741321: arr.type.names is only available for pyarrow >= 18.0.0
                # This breaks serverless workloads with client image v1/v2/v3 
                # where pyarrow is older.
                # Use the following replacement is more backward compatible.
                names=[arr.type[i].name for i in range(len(arr.type))],
                # END-EDGE
            )
        else:  # pragma: no cover
    ...
    def localize_tz(
        ...
        def convert_func(arr: pa.Array) -> pa.Array:
            ...
            # BEGIN-EDGE
            # DBRPIN-13819: pc.local_timestamp is only available for pyarrow >= 12.0.0
            # This breaks serverless workloads with client image v1/v2/v3
            # where pyarrow is older.
            if hasattr(pc, "local_timestamp"):
                return pc.local_timestamp(arr)
            return pa.array(
                [v.as_py().replace(tzinfo=None) if v.is_valid else None for v in arr],
                type=pa.timestamp(arr.type.unit),
            )
            # END-EDGE
        ...
        def convert_func(arr: pa.Array) -> pa.Array:
            ...
            if pa_type.tz is not None:
                ...
                if hasattr(pc, "local_timestamp"):
                    ...
                    arr = pa.array(
                        [
                            v.as_py().replace(tzinfo=None) if v.is_valid else None
                            for v in arr
                        ],
                        type=pa.timestamp(pa_type.unit),
                    )
                # END-EDGE
            if pa_type.unit != "ns":
    ...
    def convert_legacy(
        ...
        *,
        ...
        if df_for_struct and isinstance(spark_type, StructType):
            ...
            # BEGIN-EDGE
            # ES-1741321: arr.type.names is only available for pyarrow >= 18.0.0
            assert len(spark_type.names) == len(arr.type), (
                f"Schema mismatch: spark_type has {len(spark_type.names)} fields, "
                f"but arrow type has {len(arr.type)} fields. "
    ...
    def convert_numpy(
        ...
        *,
        ...
        if df_for_struct and isinstance(spark_type, StructType):
            ...
            # BEGIN-EDGE
            # ES-1741321: arr.type.names is only available for pyarrow >= 18.0.0
            assert len(spark_type.names) == len(arr.type), f"{spark_type} {arr.type} "
            # END-EDGE
        ...
        if isinstance(spark_type, ByteType):
            ...
            # BEGIN-EDGE
            # Keep in sync with the same conversion logic in _create_converter_to_pandas
            from pyspark._engine_adapters import IS_ENGINE_PYSPARK
            ...
            if IS_ENGINE_PYSPARK:
                ...
                series = series.map(
                    lambda v: client_pyspark_sql_types.VariantVal(v["value"], v["metadata"])
                    if v is not None
                    else None
                )
            else:
            ...
            if IS_ENGINE_PYSPARK:
                ...
                series = series.map(
                    lambda v: client_pyspark_sql_types.Geography.fromWKB(v["wkb"], v["srid"])
                    if v is not None
                    else None
                )
            else:
            ...
            if IS_ENGINE_PYSPARK:
                ...
                series = series.map(
                    lambda v: client_pyspark_sql_types.Geometry.fromWKB(v["wkb"], v["srid"])
                    if v is not None
                    else None
                )
            else:
