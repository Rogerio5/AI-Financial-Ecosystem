...
class Column(ParentColumn):
    ...
    def over(self, window: Union["WindowSpec", "RollingWindow.RollingWindowRangeSpec"]) -> ParentColumn:
        ...
        if isinstance(window, WindowSpec):
            ...
            return Column(jc)
        # BEGIN-EDGE
        elif isinstance(window, RollingWindow.RollingWindowRangeSpec):
            # For RollingWindowRangeSpec, convert to JVM object
            sc = get_active_spark_context()
            jvm = sc._jvm
            streaming = jvm.org.apache.spark.sql.streaming
            j_range = getattr(getattr(streaming, "RollingWindow$Range$"), "MODULE$").apply(
                window.preceding_range
            )
            j_spec = getattr(getattr(streaming, "RollingWindow$"), "MODULE$").preceding(j_range)
            jc = self._jc.over(j_spec)
            return Column(jc)
        # END-EDGE
        else:
            raise PySparkTypeError(
