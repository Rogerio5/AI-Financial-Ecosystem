...
class DataFrame(ParentDataFrame, PandasMapOpsMixin, PandasConversionMixin):
    ...
    def __init__(
        ...
        # BEGIN EDGE
        from pyspark.databricks.telemetry import PipelineSparkConnectMigrationTelemetry
        PipelineSparkConnectMigrationTelemetry.detect_df_construction(self)
        # END EDGE
    ...
    def rdd(self) -> "RDD[Row]":
        ...
        # BEGIN-EDGE
        from pyspark.databricks.telemetry import PipelineSparkConnectMigrationTelemetry
        PipelineSparkConnectMigrationTelemetry.detect_dataframe_to_rdd_construction()
        # END-EDGE
    ...
    def toJSON(self, use_unicode: bool = True) -> Union["RDD[str]", "DataFrame"]:
        ...
        # BEGIN-EDGE
        from pyspark.databricks.telemetry import PipelineSparkConnectMigrationTelemetry
        PipelineSparkConnectMigrationTelemetry.detect_dataframe_to_json_construction()
        # END-EDGE
    ...
    def createTempView(self, name: str) -> None:
        # BEGIN-EDGE
        from pyspark.databricks.telemetry import PipelineSparkConnectMigrationTelemetry
        PipelineSparkConnectMigrationTelemetry.detect_create_dataframe_view_in_query_fn()
        # END-EDGE
    ...
    def createOrReplaceTempView(self, name: str) -> None:
        # BEGIN-EDGE
        from pyspark.databricks.telemetry import PipelineSparkConnectMigrationTelemetry
        PipelineSparkConnectMigrationTelemetry.detect_create_replace_temp_view_mutation(name)
        # END-EDGE
    ...
    def createOrReplaceGlobalTempView(self, name: str) -> None:
        # BEGIN-EDGE
        from pyspark.databricks.telemetry import PipelineSparkConnectMigrationTelemetry
        PipelineSparkConnectMigrationTelemetry.detect_create_or_replace_global_temp_view(name)
        # END-EDGE
    ...
    def checkpoint(self, eager: bool = True) -> ParentDataFrame:
        # BEGIN-EDGE
        from pyspark.databricks.telemetry import PipelineSparkConnectMigrationTelemetry
        PipelineSparkConnectMigrationTelemetry.detect_checkpoint_command_in_query_fn()
        # END-EDGE
    ...
    def rollingWindow(
        self,
        partitionBy: Union[str, List[str], Tuple[str, ...]],
        orderBy: str,
        frontierSpec: "RollingWindow.FrontierSpec",
        measures: List[Column],
    ) -> ParentDataFrame:
        """
        Applies a rolling window aggregation on a streaming DataFrame.

        Rolling window operations compute aggregations over time-based windows that slide
        relative to a frontier. Currently, only ProcessingTime frontier mode is supported.

        .. versionadded:: 4.2.0

        Parameters
        ----------
        partitionBy : str, list of str, or tuple of str
            Column name(s) to partition the data by. Must not be empty.
        orderBy : str
            Column name to order by (typically a timestamp column). Must not be empty.
        frontierSpec : RollingWindow.FrontierSpec
            Specification for the frontier advancement.
        measures : list of Column
            List of aggregation expressions. Use `.over(RollingWindow.preceding(...))` for
            bounded rolling windows, or pass aggregations directly without `.over()` for
            lifetime windows that accumulate all historical data.

        Returns
        -------
        :class:`DataFrame`
            A new DataFrame with the rolling window aggregation applied.
        ...
        >>>
        >>> # Create rolling window specification
        >>> frontier = RollingWindow.FrontierSpec(delay="0 seconds")
        ...
        ... )  # doctest: +SKIP
        >>>
        >>> # Apply lifetime window aggregation (accumulates all historical data)
        >>> result = df.rollingWindow(
        ...     partitionBy=["value"],
        ...     orderBy="timestamp",
        ...     frontierSpec=frontier,
        ...
        Notes
        -----
        - This operation is only supported on streaming DataFrames
        - The `orderBy` column should be a timestamp column
        - Use `.over(RollingWindow.preceding(...))` for bounded windows with expiration
        - Omit `.over()` for lifetime windows that never expire data
        - The config `spark.sql.streaming.rollingWindow.enabled` must be set to true
        """
        from pyspark.sql.streaming.rolling_window import RollingWindow

        # Normalize partitionBy to a list
        if isinstance(partitionBy, str):
            partition_by_list = [partitionBy]
        elif isinstance(partitionBy, (list, tuple)):
            partition_by_list = list(partitionBy)
        else:
            raise PySparkTypeError(
                errorClass="NOT_EXPECTED_TYPE",
                messageParameters={
                    "arg_name": "partitionBy",
                    "expected_type": "list, str or tuple",
                    "arg_type": type(partitionBy).__name__,
                },
            )

        # Type validation only - let Scala handle business logic validation
        if type(orderBy) is not str:
            raise PySparkTypeError(
                errorClass="NOT_EXPECTED_TYPE",
                messageParameters={
                    "arg_name": "orderBy",
                    "expected_type": "str",
                    "arg_type": type(orderBy).__name__,
                },
            )

        if not isinstance(measures, list) or not all(isinstance(m, Column) for m in measures):
            raise PySparkTypeError(
                errorClass="NOT_EXPECTED_TYPE",
                messageParameters={
                    "arg_name": "measures",
                    "expected_type": "list[Column]",
                    "arg_type": type(measures).__name__,
                },
            )

        # Convert Python objects to Java objects
        sc = get_active_spark_context()
        jvm = sc._jvm
        assert jvm is not None

        # Convert partition columns to Scala Seq
        j_partition_by = _to_seq(sc, partition_by_list)

        # Convert FrontierSpec to Java object
        streaming = jvm.org.apache.spark.sql.streaming
        j_frontier_spec = getattr(
            getattr(streaming, "RollingWindow$FrontierSpec$"), "MODULE$"
        ).apply(frontierSpec.delay, frontierSpec.alias)

        # Convert measures to Scala Seq
        j_measures = _to_seq(sc, [_to_java_column(m) for m in measures])

        # Call the Scala rollingWindow method
        jdf = self._jdf.rollingWindow(j_partition_by, orderBy, j_frontier_spec, j_measures)
