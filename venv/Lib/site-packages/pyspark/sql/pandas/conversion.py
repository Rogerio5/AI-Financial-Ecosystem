    ...
    def _collect_as_arrow(
        ...
        try:
            if not should_use_filebase:  # Edge
                # Use the context manager to ensure the socket is always closed
                with _load_from_socket(
...
class SparkConversionMixin:
    ...
    def _create_from_pandas_with_arrow(
        ...
        # Determine spark types to coerce data when creating batches
        if isinstance(schema, StructType):
        ...
        if self._sc._conf.get(  # type: ignore[attr-defined]
            ...
            jiter = self._jvm.PythonSQLUtils.readArrowStreamFromFile(
                self._write_to_trusted_path(arrow_batches, ser)
            )
        else:
            # END-EDGE
