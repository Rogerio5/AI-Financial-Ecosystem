...
class ArrowStreamSerializer(Serializer):
    ...
    def __init__(
        ...
        # BEGIN-EDGE
        self._timely_flush_enabled = timely_flush_enabled
        self._timely_flush_timeout_ms = timely_flush_timeout_ms
        # END-EDGE

    def dump_stream(self, iterator, stream, timely_flush_enabled=None, timely_flush_timeout_ms=None):
        """Optionally prepend START_ARROW_STREAM, then write batches."""
        # BEGIN-EDGE
        if timely_flush_enabled is None:
            timely_flush_enabled = self._timely_flush_enabled
        if timely_flush_timeout_ms is None:
            timely_flush_timeout_ms = self._timely_flush_timeout_ms
        # END-EDGE
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(
            ...
            assign_cols_by_name=assign_cols_by_name,
            df_for_struct=False,
            timely_flush_enabled=True, # EDGE
            struct_in_pandas="dict",
            ndarray_as_list=False,
            prefer_int_ext_dtype=prefer_int_ext_dtype,
            arrow_cast=True,
...
class TransformWithStateInPySparkRowRealTimeModeSerializer(ArrowStreamUDFSerializer):
    ...
    def load_stream(self, stream):
        ...
        def generate_data_batches(batches):
            ...
            schema: StructType = new StructType()
                ...
                .add("timestamps", schemaForTimestamps)

            schemaForClock: StructType = new StructType()
                .add("batchTimestampMs", LongType())
                .add("eventTimeWatermarkForEviction", LongType())

            schemaForTimestamps: StructType = new StructType()
                .add("taskIngestionTS", LongType(), True)
                .add("sparkIngestionTS", LongType(), True)
                .add("sourceArrivalTS", LongType(), True)
            ...
            for batch in batches:
                ...
                timestamp_column: pa.StructArray = batch.column("timestamps")
                ...
                if batch_ts is not None:
                    ...
                    yield (
                        ...
                        # ((batch_ts, watermark_ts), (taskIngestionTS, sparkIngestionTS, sourceArrivalTS))
                        ((batch_ts, watermark_ts), [timestamp_column[0][i].as_py() for i in range(3)]),
                        None
                    )
                else:
                    for row_idx in range(batch.num_rows):
                        ...
                        timestamps = timestamp_column[row_idx]
                        yield (
                            TransformWithStateInPandasFuncMode.PROCESS_DATA,
                            # (batch_key, (taskIngestionTS, sparkIngestionTS, sourceArrivalTS))
                            (batch_key, [timestamps[i].as_py() for i in range(3)]),
                            row
                        )
    ...
    def __dump_stream_without_timeout_based_batch(self, iterator, stream):
        """
        Read through an iterator of (iterator of Row), serialize them to Arrow
        RecordBatches, and write batches to stream.
        """
        ...
        def flatten_iterator():
            ...
            for packed in iterator:
                ...
                if len(rows_as_dict) > 0:
                    fields = [
                        pa.field("data", to_arrow_type(pdf_type, timezone="UTC", prefers_large_types=False)),
                        pa.field("timestamps", to_arrow_type(StructType([
                            StructField("taskIngestionTS", LongType(), True),
                            StructField("sparkIngestionTS", LongType(), True),
                            StructField("sourceArrivalTS", LongType(), True),
                        ]), timezone="UTC", prefers_large_types=False))
                    ]

                    pdf_schema = pa.schema(fields)

                    record_batch = pa.RecordBatch.from_pylist(rows_as_dict, schema=pdf_schema)
                    yield (record_batch, pdf_schema)

        return ArrowStreamUDFSerializer.dump_stream(self, flatten_iterator(), stream)

    def __dump_stream_with_timeout_based_batch(self, iterator, stream):
        import pyarrow as pa
        import threading
        import queue

        strict_timely_flush_timeout_ms = self.batch_timeout_ms

        rows_queue = queue.Queue(maxsize=1000)
        ...
        def worker():
            ...
            try:
                ...
                for packed in iterator:
                    ...
                    for row in iter_row:
                        row_as_dict = row.asDict(True)
                        # This also plays as backpressure
                        rows_queue.put(row_as_dict)

                rows_queue.put(None)  # Signal the end of the stream
            except BaseException as e:
                rows_queue.put(e)

        threading.Thread(target=worker, daemon=True).start()

        # NOTE: We tried to simplify this via generator, but that led to ~10% performance hit.
        # Please be sure to run the benchmark if you change this code.
        class StrictTimedBatchIterator:
            def __init__(self, queue, timeout_ms):
                self.queue = queue
            ...
            def __next__(self):
                ...
                # This should provide at least one row, or None if the stream has ended.
                # That said, we will block waiting for the first row to be available.
                row = self.queue.get()
                ...
                if row is None:
                    ...
                    self.queue.task_done()
                    # Throw an exception to kill the execution in Python worker.
                    raise row
                # We have the first row, and we want to fetch more rows based on the timeout.
                rows_as_dict.append(row)
                self.queue.task_done()

                start_time_ns = time.time_ns()
                end_time_ns = start_time_ns + (self.timeout_ms * 1_000_000)

                while True:
                    remaining_time_ns = end_time_ns - time.time_ns()
                    if remaining_time_ns <= 0:
                        break

                    try:
                        row = self.queue.get(timeout=remaining_time_ns / 1000000000)
                        if row is None:  # End of stream
                            self.end_of_stream = True
                            self.queue.task_done()
                            # We don't raise StopIteration here, as we want to produce the rows
                            # we fetched before the end of stream.
                            break
                        elif isinstance(row, Exception):
                            self.queue.task_done()
                            # Throw an exception to kill the execution in Python worker.
                        ...
                        # remaining_time_ns will be negative in the next loop, no need to
                        # specifically handle this here.
                        pass

                # rows_as_dict should have at least one row.
                record_batch = pa.RecordBatch.from_pylist(rows_as_dict, schema=pdf_schema)
                return (record_batch, pdf_type)

        ArrowStreamUDFSerializer.dump_stream(
            self,
            StrictTimedBatchIterator(rows_queue, strict_timely_flush_timeout_ms),
            stream
        )
    ...
    def dump_stream(self, iterator, stream):
        ...
        if self.batch_timeout_ms <= 0:
            return self.__dump_stream_without_timeout_based_batch(iterator, stream)
        else:
            return self.__dump_stream_with_timeout_based_batch(iterator, stream)
