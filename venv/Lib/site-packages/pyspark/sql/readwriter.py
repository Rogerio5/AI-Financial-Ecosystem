...
class DataFrameReader(OptionUtils):
    ...
    def json(
        ...
        # BEGIN-EDGE
        .. connect_migration:: `spark.read.json()` does not support RDD arguments in
            Spark Connect. Collect the data first:
            `rows = [Row(**json_dict) for json_dict in json_data]`
            then `df = spark.createDataFrame(rows)`.
        # END-EDGE
        """
        self._set_opts(
            schema=schema,
    ...
    def excel(
        ...
        """Loads Excel files, returning the result as a :class:`DataFrame`.
        ...
        Examples
        --------
        Write a DataFrame into a excel file and read it back.

        >>> import tempfile
        >>> with tempfile.TemporaryDirectory(prefix="excel") as d:
        ...     # Write a DataFrame into a Excel file
        ...     spark.createDataFrame(
        ...         [{"age": 100.1, "name": "Hyukjin Kwon"}]
        ...
        if isinstance(path, str):
            path = [path]
        return self._df(self._jreader.excel(_to_seq(self._spark._sc, path)))
    # END-EDGE
...
class DataFrameWriter(OptionUtils):
    ...
    def save(
        ...
        **options : dict
        ...
        PipelineSparkConnectMigrationTelemetry.detect_write_operation_in_query_fn()
        # END-EDGE
    ...
    def excel(
        ...
        """Saves the content of the :class:`DataFrame` in excel format at the specified path.
...
class DataFrameWriterV2:
    ...
    def create(self) -> None:
        ...
        # BEGIN-EDGE
        from pyspark.databricks.telemetry import PipelineSparkConnectMigrationTelemetry
        PipelineSparkConnectMigrationTelemetry.detect_write_operation_v2_in_query_fn()
        # END-EDGE
