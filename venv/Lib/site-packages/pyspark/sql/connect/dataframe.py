...
from pyspark.sql.connect import plan # EDGE
...
class DataFrame(ParentDataFrame):
    ...
    def rollingWindow(
        ...
        """
        Rolling window aggregation for streaming DataFrames.

        .. versionadded:: 4.2.0

        Parameters
        ----------
        partitionBy : str, list, or tuple
            Column name(s) to partition by.
        orderBy : str
            Column name to order by (must be a timestamp column).
        frontierSpec : RollingWindow.FrontierSpec
            Frontier specification for the rolling window.
        measures : list of Column
            Aggregate expressions with rolling window specifications.
        ...
        # Validate measures is a list of Column objects
        ...
        for idx, measure_col in enumerate(measures):
            measure_exprs.append(measure_col._expr)

        # Create the RollingWindow plan
        return DataFrame(
            plan.RollingWindow(
                child=self._plan,
                partition_by_names=partition_by_list,
                order_by_name=orderBy,
