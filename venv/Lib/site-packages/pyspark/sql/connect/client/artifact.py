...
class ArtifactManager:
    ...
    def _get_user_context(
        self, extensions: Optional[List[any_pb2.Any]] = None
    ) -> proto.UserContext:
        """Get user context, optionally with additional extensions."""
        if not extensions:
            return self._user_context
        # Create a copy to avoid modifying the session-wide user context
        ctx = proto.UserContext()
        ctx.CopyFrom(self._user_context)
        ...
        def generator() -> Iterator[proto.AddArtifactsRequest]:
            try:
                yield from self._add_artifacts(
                    artifacts, user_context_extensions=user_context_extensions
                )
    ...
    def _add_artifacts(
        ...
        def write_batch() -> Iterator[proto.AddArtifactsRequest]:
            ...
            yield from self._add_batched_artifacts(
                current_batch, user_context_extensions=user_context_extensions
            )
            current_batch = []
            current_batch_size = 0
        ...
        for artifact in artifacts:
            ...
            if size > ArtifactManager.CHUNK_SIZE:
                ...
                yield from self._add_chunked_artifact(
                    artifact, user_context_extensions=user_context_extensions
                )
            else:
    ...
    def _add_batched_artifacts(
        ...
        yield proto.AddArtifactsRequest(
            ...
            user_context=self._get_user_context(user_context_extensions),
            batch=proto.AddArtifactsRequest.Batch(artifacts=artifact_chunks),
        )

    def _add_chunked_artifact(
        self,
        artifact: Artifact,
        user_context_extensions: Optional[List[any_pb2.Any]] = None,
    ) -> Iterator[proto.AddArtifactsRequest]:
