...
class ExecutePlanResponseReattachableIterator(Generator):
    ...
    def __init__(
        ...
        self._can_restart_execution: bool = True  # EDGE

        # By default, when the execution stream is interrupted,
        # we reestablish it with ReattachExecute. But if for whatever reason the execution
        # doesn't exist on the server side, we need to restart it with an explicit ExecutePlan
        self._restart_execution_on_next_retry: bool = False  # EDGE
    ...
    def send(self, value: Any) -> pb2.ExecutePlanResponse:
        ...
        with disable_gc():
            ...
            # BEGIN-EDGE
            # If the execution produced any meaningful response (e.g. a response schema),
            # it's unsafe to restart it in case of rejection.
            # The only response that is safe to ignore is an empty progress message.
            if not self._is_empty_execution_progress(ret):
                self._can_restart_execution = False
            # END-EDGE
    ...
    def _call_iter(self, iter_fun: Callable) -> Any:
        ...
        """
        if self._iterator is None and self._restart_execution_on_next_retry:
            self._iterator = iter(
                self._stub.ExecutePlan(self._initial_request, metadata=self._metadata)
            )
            self._restart_execution_on_next_retry = False
        ...
        try:
            ...
            if unexpected_error is not None:
                ...
                # Remove the iterator, and set the flag to restart the execution on the next retry.
                self._iterator = None
                self._restart_execution_on_next_retry = True
                raise RetryException()
            # BEGIN-EDGE
            elif (
                status is not None
                and self._extract_error_class(status) == "QUERY_REJECTED"
            ):
                if not self._can_restart_execution:
                    raise PySparkRuntimeError(
                        errorClass="QUERY_REJECTED_NOT_SAFE_TO_RESTART",
                        messageParameters={},
                    )
                # Remove the iterator, and set the flag to restart the execution on the next retry.
                self._iterator = None
                self._last_returned_response_id = None
                self._restart_execution_on_next_retry = True
                raise e
            # END-EDGE
            else:
    ...
    # BEGIN-EDGE
    @staticmethod
    def _is_empty_execution_progress(response: pb2.ExecutePlanResponse) -> bool:
        if not response.HasField("execution_progress"):
            return False
        progress = response.execution_progress
        return len(progress.stages) == 0 and progress.num_inflight_tasks == 0

    @staticmethod
    def _extract_error_class(status: Status) -> Optional[str]:
        for d in status.details:
            if d.Is(error_details_pb2.ErrorInfo.DESCRIPTOR):
                info = error_details_pb2.ErrorInfo()
                d.Unpack(info)
                return info.metadata.get("errorClass")
        return None
    # END-EDGE
