...
class DefaultPolicy(RetryPolicy):
    ...
    def can_retry(self, e: BaseException) -> bool:
        ...
        # BEGIN-EDGE
        # HTTP 404 from a non-gRPC intermediary (e.g., proxy or tunnel
        # infrastructure) is a transient routing failure, not a real
        # UNIMPLEMENTED. The gRPC library maps non-gRPC HTTP 404 to
        # UNIMPLEMENTED and includes "invalid content-type" in the
        # message since the response has text/plain instead of
        # application/grpc. Safe to retry because ReattachExecute
        # reconnects to the existing operation without re-executing.
        if e.code() == grpc.StatusCode.UNIMPLEMENTED:
            msg = str(e)
            if "invalid content-type" in msg:
                return True
        # END-EDGE
