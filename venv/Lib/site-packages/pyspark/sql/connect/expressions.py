...
# BEGIN-EDGE
class RollingWindowMeasureExpression(Expression):
    """
    Expression for rolling window measure aggregations.

    This represents an aggregate function combined with a rolling window range specification.
    It serializes to proto.Expression.rolling_window_measure and is analogous to
    RollingWindowMeasureColumn in Scala.
    """

    def __init__(
        self,
            aggregationFunction: Expression,
        rollingWindowSpec: "RollingWindow.RollingWindowRangeSpec",
    ) -> None:
        super().__init__()

        from pyspark.sql.streaming.rolling_window import RollingWindow

        assert aggregationFunction is not None and isinstance(aggregationFunction, Expression)
        assert rollingWindowSpec is not None and isinstance(
            rollingWindowSpec, RollingWindow.RollingWindowRangeSpec
        )

        self._aggregationFunction = aggregationFunction
        self._rollingWindowSpec = rollingWindowSpec

    def to_plan(self, session: "SparkConnectClient") -> proto.Expression:
        """
        Convert this RollingWindowMeasureExpression to proto.Expression.

        This creates a proto.Expression.rolling_window_measure with the aggregate
        function and the rolling window range specification.
        """
        expr = self._create_proto_expression()

        # Set the aggregate expression
        expr.rolling_window_measure.aggregate_expr.CopyFrom(
            self._aggregationFunction.to_plan(session)
        )

        # Set the range spec with duration string - server will parse it
        expr.rolling_window_measure.range_spec.preceding_range_duration = (
            self._rollingWindowSpec.preceding_range
        )
    ...
    @property
    def children(self) -> Sequence["Expression"]:
        return [self._aggregationFunction]

    def __repr__(self) -> str:
        return f"RollingWindowMeasureExpression({str(self._aggregationFunction)}.over{str(self._rollingWindowSpec)})"
# END-EDGE
