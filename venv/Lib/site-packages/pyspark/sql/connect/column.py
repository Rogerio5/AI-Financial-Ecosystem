...
class Column(ParentColumn):
    ...
    def over(self, window: Union["WindowSpec", "RollingWindow.RollingWindowRangeSpec"]) -> ParentColumn:  # type: ignore[override]
        from pyspark.sql.connect.window import WindowSpec
        from pyspark.sql.streaming.rolling_window import RollingWindow
        from pyspark.sql.connect.expressions import RollingWindowMeasureExpression
        ...
        # BEGIN-EDGE
            ...
            return Column(
                RollingWindowMeasureExpression(aggregationFunction=self._expr, rollingWindowSpec=window)
            )
        # END-EDGE
        else:
            raise PySparkTypeError(
