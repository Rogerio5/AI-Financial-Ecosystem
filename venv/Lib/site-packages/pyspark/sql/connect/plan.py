...
if TYPE_CHECKING:
    ...
    from pyspark.sql.streaming.rolling_window import RollingWindow # EDGE
...
# BEGIN-EDGE
class RollingWindow(LogicalPlan):
    """Logical plan object for a RollingWindow operation."""

    def __init__(
        self,
        child: Optional["LogicalPlan"],
        partition_by_names: List[str],
        order_by_name: str,
        frontier_spec: "RollingWindow.FrontierSpec",
        measures: List[Expression],
    ):
        super().__init__(child)
        self._partition_by_names = partition_by_names
        self._order_by_name = order_by_name
        self._frontier_spec = frontier_spec
    ...
    def plan(self, session: "SparkConnectClient") -> proto.Relation:
        ...
        # Set input relation
        plan.rolling_window.input.CopyFrom(self._child.plan(session))

        # Set partition columns
        plan.rolling_window.partition_by_names.extend(self._partition_by_names)

        # Set order column
        plan.rolling_window.order_by_name = self._order_by_name

        # Set frontier spec - pass duration string, server will parse it
        frontier_proto = plan.rolling_window.frontier_spec
        frontier_proto.time_mode = "ProcessingTime"  # Currently only ProcessingTime supported
        frontier_proto.delay_duration = self._frontier_spec.delay
        frontier_proto.alias = self._frontier_spec.alias

        # Set measures - each measure is a RollingWindowMeasureExpression
        for measure in self._measures:
            plan.rolling_window.measures.append(measure.to_plan(session))
