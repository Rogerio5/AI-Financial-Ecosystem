...
class StreamingQuery:
    ...
    # Note: Read-only property accessors (isActive, id, runId, name) are intentionally not
    # instrumented with telemetry because they are safe/supported in Spark Connect and do not
    # represent write or control operations that need migration tracking.
    ...
    def awaitTermination(self, timeout: Optional[int] = None) -> Optional[bool]:
        ...
        PipelineSparkConnectMigrationTelemetry.detect_streaming_query_command_in_query_fn()
        # END-EDGE
...
class StreamingQueryManager:
    ...
    def active(self) -> List[StreamingQuery]:
        ...
        # BEGIN-EDGE
        from pyspark.databricks.telemetry import PipelineSparkConnectMigrationTelemetry
        PipelineSparkConnectMigrationTelemetry.detect_streaming_query_manager_command_in_query_fn()
        # END-EDGE
    ...
    def addListener(self, listener: StreamingQueryListener) -> None:
        ...
        # BEGIN-EDGE
        from pyspark.databricks.telemetry import PipelineSparkConnectMigrationTelemetry
        PipelineSparkConnectMigrationTelemetry.detect_streaming_query_listener_bus_command_in_query_fn()
        # END-EDGE
...
class StreamingCheckpointManager:
    ...
    def rewindToBatch(self, checkpoint_location: str, batch_id: int, config: Dict[str, str] = {}) -> None:
        self._jmanager.rewindToBatch(checkpoint_location, batch_id, config)

    def rewindToLastCommittedBatch(self, checkpoint_location: str, config: Dict[str, str] = {}) -> None:
        self._jmanager.rewindToLastCommittedBatch(checkpoint_location, config)

    def rewindToTimestampMs(self, checkpoint_location: str, timestamp_ms: int, config: Dict[str, str] = {}) -> None:
        self._jmanager.rewindToTimestampMs(checkpoint_location, timestamp_ms, config)

    def rewindToSourceOffset(self, checkpoint_location: str, source_id: str, json: str, config: Dict[str, str] = {}) -> None:
        self._jmanager.rewindToSourceOffset(checkpoint_location, source_id, json, config)
