...
from pyspark.sql.streaming.write_buffer_state_ipc_processor import WriteBufferStateIpcProcessor
# END-EDGE
...
class StatefulProcessorApiClient:
    def __init__(
        ...
        worker_cache_enabled: bool = False, # EDGE
    ) -> None:
        ...
        elif isinstance(state_server_id, str):
            self._client_socket = socket.socket(socket.AF_UNIX, socket.SOCK_STREAM)
            self._client_socket.connect(state_server_id)
        else:
        ...
        self.state_ipc_processor: Optional[StateIpcProcessor] = (
            WriteBufferStateIpcProcessor(self)
            if (worker_cache_enabled and is_realtime_mode)
            else None
        )
        # END-EDGE
    ...
    def register_timer(self, expiry_time_stamp_ms: int) -> None:
        import pyspark.sql.streaming.proto.StateMessage_pb2 as stateMessage
        # BEGIN-EDGE
        if self.state_ipc_processor is not None:
            self.state_ipc_processor.buffer_timer(
                expiry_time_stamp_ms, TimerWriteBufferOperation.REGISTER
            )
            return
        # END-EDGE
    ...
    def delete_timer(self, expiry_time_stamp_ms: int) -> None:
        ...
        if self.state_ipc_processor is not None:
            self.state_ipc_processor.buffer_timer(
                expiry_time_stamp_ms, TimerWriteBufferOperation.DELETE
            )
            return
        # END-EDGE
...
class ListTimerIterator:
    def __init__(self, stateful_processor_api_client: StatefulProcessorApiClient):
        ...
        # BEGIN-EDGE
        if self.stateful_processor_api_client.state_ipc_processor is not None:
            self.stateful_processor_api_client.state_ipc_processor.on_iterator_start()
        # END-EDGE
