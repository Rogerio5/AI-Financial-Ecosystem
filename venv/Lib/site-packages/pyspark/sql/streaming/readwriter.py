...
class DataStreamReader(OptionUtils):
    ...
    def name(self, source_name: str) -> "DataStreamReader":
        ...
        This allows streaming queries to evolve by enabling sources to be reordered
        or added without breaking checkpoint compatibility. When source evolution is
        enabled, all sources in a query must be named.

        Parameters
        ----------
        source_name : str
            The name for this streaming source. Must contain only ASCII letters (a-z, A-Z),
            digits (0-9), and underscores (_). Names are used as directory names in the
            checkpoint location, so they must be filesystem-safe.

        Returns
        -------
        DataStreamReader
            The DataStreamReader for method chaining.

        Notes
        -----
        This API requires streaming source evolution to be enabled via the
        ``spark.sql.streaming.enableSourceEvolution`` configuration. This API is evolving.

        Examples
        --------
        >>> df1 = spark.readStream.format("rate").name("source1").load()  # doctest: +SKIP
        >>> df2 = spark.readStream.format("rate").name("source2").load()  # doctest: +SKIP
        >>> query = df1.union(df2).writeStream.format("console").start()  # doctest: +SKIP

        Source names must follow naming rules:

        >>> # Valid names
        >>> spark.readStream.format("rate").name("mySource").load()  # doctest: +SKIP
        >>> spark.readStream.format("rate").name("my_source_123").load()  # doctest: +SKIP
        >>> spark.readStream.format("rate").name("_privateSource").load()  # doctest: +SKIP

        Invalid names will raise an error:

        >>> spark.readStream.format("rate").name("my-source").load()  # doctest: +SKIP
        Traceback (most recent call last):
            ...
        AnalysisException: Invalid source name 'my-source'...
        """
...
class DataStreamWriter:
    ...
    def start(
        ...
        **options : dict
        ...
        PipelineSparkConnectMigrationTelemetry.detect_write_stream_start_in_query_fn()
        # END-EDGE
        self.options(**options)
