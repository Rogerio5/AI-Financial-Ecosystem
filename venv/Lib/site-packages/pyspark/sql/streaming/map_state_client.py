...
class MapStateClient:
    def __init__(
        ...
        # BEGIN-EDGE
        processor = self._stateful_processor_api_client.state_ipc_processor
        self._state_ipc_processor = (
            processor
            if (processor is not None and (not is_ttl_state or processor.supports_ttl_state))
            else None
        )
        # END-EDGE
    ...
    def exists(self, state_name: str) -> bool:
        ...
        # BEGIN-EDGE
        if self._state_ipc_processor is not None:
            result = self._state_ipc_processor.map_state_exists(state_name)
    ...
    def get_value(self, state_name: str, user_key: Tuple) -> Optional[Tuple]:
        ...
        # BEGIN-EDGE
        if self._state_ipc_processor is not None:
            result = self._state_ipc_processor.map_state_get_value(
                state_name, user_key, self.user_key_schema, self.value_schema,
            )
            if result is not None and result[1]:
                # result is a (value, handled) tuple, only returns value if handled is True.
                return result[0]
        # END-EDGE
    ...
    def contains_key(self, state_name: str, user_key: Tuple) -> bool:
        ...
        # BEGIN-EDGE
        if self._state_ipc_processor is not None:
            result = self._state_ipc_processor.map_state_contains_key(
                state_name, user_key, self.user_key_schema,
            )
            if result is not None:
                return result
        # END-EDGE
    ...
    def update_value(
        ...
        # BEGIN-EDGE
        if self._state_ipc_processor is not None:
            return self._state_ipc_processor.map_state_update_value(
                state_name, user_key, value,
                self.user_key_schema, self.value_schema,
            )
        # END-EDGE
    ...
    def remove_key(self, state_name: str, key: Tuple) -> None:
        ...
        # BEGIN-EDGE
        if self._state_ipc_processor is not None:
            return self._state_ipc_processor.map_state_remove_key(
                state_name, key, self.user_key_schema, self.value_schema,
            )
        # END-EDGE
    ...
    def clear(self, state_name: str) -> None:
        ...
        # BEGIN-EDGE
        if self._state_ipc_processor is not None:
            return self._state_ipc_processor.map_state_clear(state_name)
        # END-EDGE
...
class MapStateIterator:
    def __init__(self, map_state_client: MapStateClient, state_name: str, is_key: bool):
        ...
        # BEGIN-EDGE
        if self.map_state_client._state_ipc_processor is not None:
            self.map_state_client._state_ipc_processor.on_iterator_start()
        # END-EDGE
