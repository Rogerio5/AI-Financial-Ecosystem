...
class TransformWithStateInPandasUdfUtils:
    ...
    def __init__(self, stateful_processor: StatefulProcessor, time_mode: str):
        ...
        # This variable is used for processing time timer latency tracking in real-time mode
        self.current_timer_expiry_timestamp = None
    ...
    def _transformWithStateUDF_realtime_mode(
        ...
        if mode == TransformWithStateInPandasFuncMode.PROCESS_TIMER:
            ...
            # BEGIN-EDGE
            if stateful_processor_api_client.state_ipc_processor is not None:
                stateful_processor_api_client.state_ipc_processor.on_partition_close()
            else:
                # END-EDGE
                stateful_processor_api_client.remove_implicit_key()
            self._stateful_processor.close()
            ...
            # key should be ((batch_timestamp, watermark_timestamp), (taskIngestionTS, sparkIngestionTS, sourceArrivalTS))
            batch_timestamp, watermark_timestamp = key[0]
            ...
            for result in self._handle_expired_timers(stateful_processor_api_client):
                if self._time_mode.lower() == "processingtime":
                    taskIngestionTS = batch_timestamp
                    sparkIngestionTS = batch_timestamp
                    sourceArrivalTS = self.current_timer_expiry_timestamp
                else:
                    # none or event time mode
                    taskIngestionTS, sparkIngestionTS, sourceArrivalTS = key[1]
                yield Row(data = result, timestamps = Row(taskIngestionTS = taskIngestionTS, sparkIngestionTS = sparkIngestionTS, sourceArrivalTS = sourceArrivalTS))

            if self._time_mode.lower() == "processingtime":
                stateful_processor_api_client.cleanup_ttl()
        else:
            # mode == TransformWithStateInPandasFuncMode.PROCESS_DATA
            # key should be (grouping_key, (taskIngestionTS, sparkIngestionTS, sourceArrivalTS))
            taskIngestionTS, sparkIngestionTS, sourceArrivalTS = key[1]
            for result in self._handle_data_rows(stateful_processor_api_client, key[0], input_rows):
                yield Row(data = result, timestamps = Row(taskIngestionTS = taskIngestionTS, sparkIngestionTS = sparkIngestionTS, sourceArrivalTS = sourceArrivalTS))
    ...
    def _handle_data_rows(
        ...
        # BEGIN-EDGE
        if stateful_processor_api_client.state_ipc_processor is not None:
            stateful_processor_api_client.state_ipc_processor.on_new_grouping_key(key)
        else:
            # END-EDGE
            stateful_processor_api_client.set_implicit_key(key)
    ...
    def _handle_expired_timers(
        ...
        for key_obj, expiry_timestamp in expiry_iter:
            self.current_timer_expiry_timestamp = expiry_timestamp
            # BEGIN-EDGE
            if stateful_processor_api_client.state_ipc_processor is not None:
                stateful_processor_api_client.state_ipc_processor.on_new_grouping_key(key_obj)
            else:
                # END-EDGE
        ...
        # BEGIN-EDGE
        if stateful_processor_api_client.state_ipc_processor is not None:
            stateful_processor_api_client.state_ipc_processor.on_timers_complete()
        # END-EDGE
