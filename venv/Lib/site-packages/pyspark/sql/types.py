...
class GeographyType(SpatialType):
    ...
    def __init__(self, srid):  # EDGE
        ...
        # EDGE TODO(GEO-560): Account for more SRIDs here, once they are supported.
        elif not isinstance(srid, int) or srid != GeographyType.DEFAULT_SRID:  # EDGE
            ...
            self.srid = srid
            self._crs = GeographyType.DEFAULT_CRS  # EDGE
...
class UserDefinedType(DataType):
    ...
    def fromJson(cls, json: Dict[str, Any]) -> "UserDefinedType":
        ...
        session = SparkSession.getActiveSession()
        if session:
            allow_unpickle = session.conf.get(
                "spark.databricks.pyspark.udt.allowUnpickle",
                "true",
            ).lower() == "true"
            allow_import = session.conf.get(
                "spark.databricks.pyspark.udt.allowImport",
                "true",
            ).lower() == "true"
            import_type_check = session.conf.get(
                "spark.databricks.pyspark.udt.importTypeCheck",
                "false",
            ).lower() == "true"
            log_mode = session.conf.get(
                "spark.databricks.pyspark.udt.log",
                "false",
            ).lower() == "true"
        else:
            # Without session, this should behave as before we introduced the new confs
            allow_unpickle = True
            allow_import = True
            import_type_check = False
            log_mode = False

        logger = None
        if log_mode:
            try:
                from pyspark.databricks.usage_logger import get_logger
                logger = get_logger()
            except Exception:
                logger = None
        ...
        if not hasattr(m, pyClass):
            if logger:
                logger.log_success(
                    ...
                    "fromJson",
                    0,
                    f"(pickle, {allow_unpickle})"
                )
            if allow_unpickle:
                # We allow unpickling for "enable" or "log"
            ...
            if allow_import:
                ...
                if not (isinstance(UDT, type) and issubclass(UDT, UserDefinedType)):
                    if logger:
                        logger.log_success(
                            ...
                            "fromJson",
                            0,
                            f"(typemismatch, {import_type_check})",
                        )
                    if import_type_check:
                        raise PySparkTypeError(
                ...
                raise PySparkValueError(
                    errorClass="UNSUPPORTED_OPERATION",
                    messageParameters={"operation": "importing user defined types"},
                )
        return UDT()
