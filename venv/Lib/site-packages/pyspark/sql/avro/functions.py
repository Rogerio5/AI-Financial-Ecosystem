...
def from_avro(
    ...
    typecheck_enabled = (
        str(activeSession.conf.get("spark.databricks.sql.avro.pythonAvroTypecheck.enabled")).lower()
        == "true"
    )
...
def to_avro(
    ...
    ...   .select(to_avro("str", None, lit("input"), schemaRegistryAddress, options)
    ...           .alias("value")).collect()
    [Row(value=b'\x00\x00\x00\x00\x01\x020'),
     Row(value=b'\x00\x00\x00\x00\x01\x021'),
     Row(value=b'\x00\x00\x00\x00\x01\x022'),
     Row(value=b'\x00\x00\x00\x00\x01\x023'),
     Row(value=b'\x00\x00\x00\x00\x01\x024')]
