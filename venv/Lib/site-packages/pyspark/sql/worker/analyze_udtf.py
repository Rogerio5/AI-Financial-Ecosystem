...
def read_udtf(infile: IO) -> type[UDTFHandler]:
    ...
    # BEGIN-EDGE
    # Check if this is a UC UDTF code literal by reading the length marker
    from pyspark.sql.worker.read_uc_udtf_analyze import read_uc_udtf_handler

    length = read_int(infile)
    if length == SpecialLengths.PYTHON_CODE_LITERAL:
        handler = read_uc_udtf_handler(infile, length)
    else:
        handler = pickleSer._read_with_known_length(infile, length)
    # END-EDGE
...
def _main(infile: IO, outfile: IO) -> None:
    ...
    _main.__hide_traceback__ = udtf_name == "ai_forecast" or None
    # END-EDGE
