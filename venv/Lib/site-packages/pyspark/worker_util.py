...
def check_python_version_with_flag(infile: IO) -> None:
    """
    Planner workers may prepend a boolean flag to indicate whether version checking is needed.
    """
    if read_bool(infile):
...
def monkey_patch_credential_chain_if_needed():
    ...
    UDFs.
    """
    if not (os.getenv("DB_UC_SECURABLE_SOCKET_PATH") or os.getenv("SERVICE_CREDENTIAL_SOCKET_PATH")):
