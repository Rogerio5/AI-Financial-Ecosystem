...
def handle_worker_exception(
    ...
    try:
        # BEGIN-EDGE
        # Check for UCSecurableError first — these carry structured error info
        # from the UC securable server and should be written as structured exceptions
        # for both taco and burrito codepaths. Use duck typing to avoid import
        # dependency on databricks._base_grpc_client which may not be on sys.path.
        is_uc_securable_error = (
            hasattr(e, 'error_class') and hasattr(e, 'subclass')
            and hasattr(e, 'message_parameters') and hasattr(e, 'originating_in_payload'))

        if is_uc_securable_error:
            from pyspark.serializers import write_structured_exception
            error_class = f"{e.error_class}.{e.subclass}" if e.subclass else e.error_class
            write_structured_exception(
                ...
                stack_lines=[],
                stream=outfile,
                message_parameters=e.message_parameters,
            )
            return
...
class _FaulthandlerHelper:
    def __init__(self) -> None:
        ...
        self._use_stderr = False # EDGE
        self._log_file: Optional[TextIO] = None
        self._periodic_traceback = False
        self._reentry_depth = 0
        self._react_on_sigquit: Optional[str] = None # EDGE
        self._sigquit_handler_file: Optional[TextIO] = None # EDGE
        self._sigquit_handler_filename: Optional[str] = None # EDGE
    ...
    def start(self) -> None:
        ...
        self._use_stderr = os.environ.get("PYTHON_FAULTHANDLER_USE_STDERR", False) # EDGE
        self._react_on_sigquit = os.environ.get("PYTHON_FAULTHANDLER_SIGQUIT", None) # EDGE
        if self._log_path:
        ...
        if self._react_on_sigquit:
            # There is a request to react on a sigquit, therefore first ensure that any parent
            # process did not block this signal.
            try:
                if signal.SIGQUIT in signal.pthread_sigmask(signal.SIG_BLOCK, []):
                    signal.pthread_sigmask(signal.SIG_UNBLOCK, [signal.SIGQUIT])
            except AttributeError:
                print("pthread_sigmask not available, cannot unblock signals")
            if self._react_on_sigquit == 'stdout':
                faulthandler.register(signal.SIGQUIT, file=sys.stdout, all_threads=True, chain=False)
            elif self._react_on_sigquit.startswith('path:'):
                self._sigquit_handler_filename = os.path.join(
                    self._react_on_sigquit[len('path:'):],
                    "thread_dump_{}".format(os.getpid()),
                )
                try:
                    self._sigquit_handler_file = open(self._sigquit_handler_filename, "wb", buffering=0)
                    faulthandler.register(signal.SIGQUIT, file=self._sigquit_handler_file, all_threads=True, chain=False)
                except Exception as e:
                    print("Failed to set the registration for sigquit handler: {}".format(e))
                    # If the registration of signal  handler failed - we should
                    # disable reaction on this signal, as by default it kills process.
    ...
    def stop(self) -> None:
        ...
        if self._react_on_sigquit:
            ...
            if self._sigquit_handler_file:
                self._sigquit_handler_file.close()
                self._sigquit_handler_file = None
            if self._sigquit_handler_filename:
                try:
                    # remove signal handler, if unregister was a no op.
                    # there are no implications of several calls
                    signal.signal(signal.SIGQUIT, signal.SIG_DFL)
                except Exception as e:
                    print("Failed to restore default signal handler: {}".format(e))
                try:
                    os.remove(self._sigquit_handler_filename)
                except Exception as e:
                    print("Failed to remove sigquit handler file: {}".format(e))
                finally:
                    self._sigquit_handler_filename = None
            self._react_on_sigquit = False
        # END EDGE
