...
if IS_ENGINE_PYSPARK or ENGINE_PYSPARK_OVERRIDE:
    ...
    # _parse schema json into client-side DataType
    def _client_parse_datatype_json_string(schema_json):
        from pyspark.sql.types import _parse_datatype_json_string
        return _parse_datatype_json_string(schema_json)
    ...
    def _set_task_context_fields(ctx, json):
        from pyspark.resource import ResourceInformation
        # Set the fields of the task context from the json.
        ctx._stageId = json["stageId"]
        ctx._partitionId = json["partitionId"]
        ctx._attemptNumber = json["attemptNumber"]
        ctx._taskAttemptId = json["taskAttemptId"]
        ctx._cpus = json["cpus"]
        ctx._resources = {
            k: ResourceInformation(v["name"], v["addresses"])
            for k, v in json["resources"].items()
        },
        ctx._localProperties = json["localProperties"]
        return ctx


    # ES-1720528: be backward compatible for client task context that does not have from_json method.
    def get_client_task_context_from_json(json):
        # Fallback to the json initialization if possible.
        if (hasattr(client_pyspark_taskcontext.TaskContext ,"from_json")):
            return client_pyspark_taskcontext.TaskContext.from_json(json)
        ctx = client_pyspark_taskcontext.TaskContext._getOrCreate()
        return _set_task_context_fields(ctx, json)

    # ES-1720528: be backward compatible for client barrier task context that does not have from_json method.
    def get_client_barrier_task_context_from_json(json):
        # Fallback to the json initialization if possible.
        if (hasattr(client_pyspark_taskcontext.BarrierTaskContext ,"from_json")):
            return client_pyspark_taskcontext.BarrierTaskContext.from_json(json)
        client_pyspark_taskcontext.BarrierTaskContext._initialize(json["connInfo"], json["secret"])
        ctx = client_pyspark_taskcontext.BarrierTaskContext._getOrCreate()
        return _set_task_context_fields(ctx, json)
