...
Each PySpark module's docstring may contain one or more of the following
reStructuredText directives. These annotate APIs as compatible with Spark Connect or not. In some
cases we also include a migration message to guide the user on how to migrate to Spark Connect.

  ``.. classic:: true``
      This module (or class / method) is not compatible with Spark Connect.

  ``.. connect:: true``
      This module (or class / method) is compatible with Spark Connect.

  ``.. connect_migration:: <message>``
      Human-readable guidance shown in the IDE when a user calls this API
      from a Spark Connect context.

  ``.. serverless_migration:: <message>``
      Variant of ``connect_migration`` shown specifically in Serverless contexts.

To be terse, annotations are resolved by inheriting from the nearest-annotated ancestor.
A child's annotation and message override the parent's.
...
if IS_ENGINE_PYSPARK:
    # Import the engine_pyspark early to avoid circular imports
    from pyspark._engine_adapters import setup_pyspark_pickle_serializer
    setup_pyspark_pickle_serializer()
# END-EDGE
...
# BEGIN-EDGE
from pyspark.databricks.instrumentation.instrumentation_utils import (
    _instrument_after_spark_context_init,
    _ExtendedInstrumentationConfig,
)
...
def _get_extended_instrumentation_config():
    ...
    return _ExtendedInstrumentationConfig(
        ...
        instance_var_allow_list=[
            (Broadcast, "_jbroadcast"),
            (Broadcast, "_sc"),
            (SparkConf, "_jconf"),
            (SparkContext, "_jsc"),
            (SparkContext, "_javaAccumulator"),
            (StatusTracker, "_jtracker"),
        ],
        class_var_allow_list=[
            (SparkContext, "_jvm"),
