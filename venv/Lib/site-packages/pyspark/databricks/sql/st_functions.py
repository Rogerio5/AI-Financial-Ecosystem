...
from pyspark.sql import functions as ossf
...
@try_remote_edge_functions
def st_azimuth(col1: "ColumnOrName", col2: "ColumnOrName") -> Column:
    ...
    ----------
    col1 : :class:`~pyspark.sql.Column` or str
        The first GEOGRAPHY or GEOMETRY value.
    col2 : :class:`~pyspark.sql.Column` or str
        The second GEOGRAPHY or GEOMETRY value.

    Notes
    -----
    The two inputs are expected to be points, otherwise an error is returned. The two input points
    are expected to have the same SRID value, otherwise an error is returned. If any of the two
    input points is empty, `null` is returned.
    This is an EDGE feature that is only supported in DBR.
    ...
    >>> df.select(round(dbf.st_azimuth(dbf.st_geomfromtext('wkt1'), dbf.st_geomfromtext('wkt2')), 6).alias('result')).collect() # noqa
    [Row(result=0.785398)]
    >>> df.select(round(dbf.st_azimuth(dbf.st_geogfromtext('wkt1'), dbf.st_geogfromtext('wkt2')), 6).alias('result')).collect() # noqa
    [Row(result=0.784083)]
    """
    sc = SparkContext._active_spark_context
    assert sc is not None and sc._jvm is not None
    jc = sc._jvm.com.databricks.sql.functions.st_azimuth(
        to_java_column(col1), to_java_column(col2)
    )
    return Column(jc)
...
def st_boundary(col: "ColumnOrName") -> Column:
    ...
    - If the input is a linestring, its boundary is the empty multipoint if it is closed or the
      non-empty multipoint consisting of its two endpoints if it is not closed.
    - If the input is a multilinestring, its boundary is the possibly empty multipoint that
      consists of all the boundary points of all its non-closed linestrings with odd parity
      (mod-2 rule).
    - If the input is a polygon, its boundary is the linestring or multilinestring containing
      the rings of the polygon.
    - If the input is a multipolygon, its boundary is the linestring or multilinestring containing
      the rings of all the polygons in the multipolygon.
    - For collections, the boundary can be a multipoint, linestring, multilinestring, or a
      collection containing a multipoint and linestring/multilinestring.
    This is an EDGE feature that is only supported in DBR.
    ...
    >>> df.select(dbf.st_astext(dbf.st_boundary(dbf.st_geomfromtext('wkt'))).alias('result')).collect()  # noqa
    [Row(result='MULTILINESTRING((0 0,10 0,10 10,0 10,0 0),(2 2,8 2,8 8,2 8,2 2))')]
    """
    sc = SparkContext._active_spark_context
    assert sc is not None and sc._jvm is not None
    jc = sc._jvm.com.databricks.sql.functions.st_boundary(
        to_java_column(col)
    )
    return Column(jc)
...
def st_closestpoint(col1: "ColumnOrName", col2: "ColumnOrName") -> Column:
    """Returns the 2D projection of a point in the first GEOMETRY value that is closest to the
    second GEOMETRY value with respect to the Euclidean distance.
    ...
    --------
    >>> from pyspark.databricks.sql import functions as dbf
    >>> df = spark.createDataFrame([('MULTIPOINT(-10 10,1 10,20 10)','POINT(0 0)',)], ['wkt1', 'wkt2'])
    >>> df.select(dbf.st_astext(dbf.st_closestpoint(dbf.st_geomfromtext('wkt1'), dbf.st_geomfromtext('wkt2'))).alias('result')).collect() # noqa
    [Row(result='POINT(1 10)')]
    """
    sc = SparkContext._active_spark_context
    assert sc is not None and sc._jvm is not None
    jc = sc._jvm.com.databricks.sql.functions.st_closestpoint(
        to_java_column(col1), to_java_column(col2)
    )
    return Column(jc)
...
@try_remote_edge_functions
def st_estimatesrid(col: "ColumnOrName") -> Column:
    """Returns the best projected SRID (UTM or UPS) for the input GEOMETRY based on the center of
    ...
    ----------
    col : :class:`~pyspark.sql.Column` or str
        A GEOMETRY value with coordinates in degrees in the range [-180, 180] for longitudes
    ...
    sc = SparkContext._active_spark_context
    assert sc is not None and sc._jvm is not None
    jc = sc._jvm.com.databricks.sql.functions.st_estimatesrid(to_java_column(col))
    return Column(jc)
...
@try_remote_edge_functions
def st_force2d(col: "ColumnOrName") -> Column:
    """Returns the 2D projection of the input geography or geometry.
    ...
    The SRID value of the output is the same as that of the input value.
    This is an EDGE feature that is only supported in DBR.

    Examples
    --------
    Example 1: Convert a 3DM point geography to 2D.
    >>> from pyspark.databricks.sql import functions as dbf
    >>> df = spark.createDataFrame([('POINT M (1 2 3)',)], ['wkt'])
    >>> df.select(dbf.st_astext(dbf.st_force2d(dbf.st_geogfromtext('wkt'))).alias('result')).collect()  # noqa
    [Row(result='POINT(1 2)')]

    Example 2: Convert a 3DZ polygon geography to 2D.
    >>> from pyspark.databricks.sql import functions as dbf
    >>> df = spark.createDataFrame([('POLYGON Z ((0 0 2,1 0 3,0 1 4,0 0 5))',)], ['wkt'])
    >>> df.select(dbf.st_astext(dbf.st_force2d(dbf.st_geogfromtext('wkt'))).alias('result')).collect()  # noqa
    [Row(result='POLYGON((0 0,1 0,0 1,0 0))')]

    Example 3: Convert a 3DZ point geometry to 2D.
    >>> from pyspark.databricks.sql import functions as dbf
    >>> df = spark.createDataFrame([('POINT Z (1 2 3)',)], ['wkt'])
    >>> df.select(dbf.st_astext(dbf.st_force2d(dbf.st_geomfromtext('wkt'))).alias('result')).collect()  # noqa
    [Row(result='POINT(1 2)')]

    Example 4: Convert a 4D linestring geometry to 2D.
    >>> from pyspark.databricks.sql import functions as dbf
    >>> df = spark.createDataFrame([('LINESTRING ZM (0 1 -1 100,1 0 -2 200,2 -1 17 500)',)], ['wkt'])
    >>> df.select(dbf.st_astext(dbf.st_force2d(dbf.st_geomfromtext('wkt'))).alias('result')).collect()  # noqa
    [Row(result='LINESTRING(0 1,1 0,2 -1)')]
    """
    sc = SparkContext._active_spark_context
    assert sc is not None and sc._jvm is not None
    jc = sc._jvm.com.databricks.sql.functions.st_force2d(to_java_column(col))
    return Column(jc)


@try_remote_edge_functions
def st_geogfromewkt(col: "ColumnOrName") -> Column:
    """Parses the EWKT description and returns the corresponding GEOGRAPHY value. The function
    returns an error if the SRID specification is missing.
    ...
    ----------
    col : :class:`~pyspark.sql.Column` or str
        A STRING value in EWKT format, representing a GEOGRAPHY value.
    ...
    --------
    >>> from pyspark.databricks.sql import functions as dbf
    >>> df = spark.createDataFrame([('SRID=4326;LINESTRING(5 6,7 -8)',)], ['ewkt'])
    >>> df.select(dbf.st_asewkt(dbf.st_geogfromewkt('ewkt')).alias('result')).collect()
    [Row(result='SRID=4326;LINESTRING(5 6,7 -8)')]
    """
    sc = SparkContext._active_spark_context
    assert sc is not None and sc._jvm is not None
    jc = sc._jvm.com.databricks.sql.functions.st_geogfromewkt(to_java_column(col))
    return Column(jc)
...
def st_geogfromwkb(col: "ColumnOrName") -> Column:
    ...
    [Row(result='POLYGON((0 0,5 6,7 -8,0 0))')]
    """
    return ossf.st_geogfromwkb(col)
...
@try_remote_edge_functions
def st_geomfromewkt(col: "ColumnOrName") -> Column:
    """Parses the EWKT description and returns the corresponding GEOMETRY value.
    ...
    ----------
    col : :class:`~pyspark.sql.Column` or str
        A STRING value in EWKT format, representing a GEOMETRY value.
    ...
    >>> df.select(dbf.st_asewkt(dbf.st_geomfromewkt('wkt')).alias('result')).collect()
    [Row(result='LINESTRING(5 6,7 -8)')]
    >>> from pyspark.databricks.sql import functions as dbf
    >>> df = spark.createDataFrame([('SRID=3857;LINESTRING(5 6,7 -8)',)], ['ewkt'])
    >>> df.select(dbf.st_asewkt(dbf.st_geomfromewkt('ewkt')).alias('result')).collect()
    [Row(result='SRID=3857;LINESTRING(5 6,7 -8)')]
    """
    sc = SparkContext._active_spark_context
    assert sc is not None and sc._jvm is not None
    jc = sc._jvm.com.databricks.sql.functions.st_geomfromewkt(to_java_column(col))
    return Column(jc)
...
def st_geomfromwkb(
    ...
    [Row(result='SRID=4326;LINESTRING(5 6,7 -8)')]
    """
    return ossf.st_geomfromwkb(col1, col2)
...
@try_remote_edge_functions
def st_makeenvelope(
    x1: "ColumnOrName", y1: "ColumnOrName", x2: "ColumnOrName", y2: "ColumnOrName"
) -> Column:
    """Returns a GEOMETRY representing the axis-aligned envelope defined by the two corner
    coordinates (x1, y1) and (x2, y2). The result is a POINT, LINESTRING, or POLYGON depending
    on whether the box degenerates to a point, segment, or rectangle. The SRID of the returned
    geometry is 0.
    ...
    Parameters
    ----------
    x1 : :class:`~pyspark.sql.Column` or float
        The first corner X coordinate.
    y1 : :class:`~pyspark.sql.Column` or float
        The first corner Y coordinate.
    x2 : :class:`~pyspark.sql.Column` or float
        The second corner X coordinate.
    y2 : :class:`~pyspark.sql.Column` or float
        The second corner Y coordinate.
    ...
    sc = SparkContext._active_spark_context
    assert sc is not None and sc._jvm is not None
    jc = sc._jvm.com.databricks.sql.functions.st_makeenvelope(
        to_java_column(x1), to_java_column(y1), to_java_column(x2), to_java_column(y2)
    )
    return Column(jc)
...
@try_remote_edge_functions
def st_makepoint(*cols: "ColumnOrName") -> Column:
    """Returns a point GEOMETRY with the given coordinates. Takes 2 (x, y), 3 (x, y, z), or 4
    (x, y, z, m) double values. The SRID of the returned geometry is 0.

    .. versionadded:: 4.1.0

    Parameters
    ----------
    *cols : :class:`~pyspark.sql.Column` or float
    ...
    Example 2: Create a 3DZ point.
    >>> from pyspark.databricks.sql import functions as dbf
    >>> df = spark.createDataFrame([(10.0, 34.0, 1.0)], ['x', 'y', 'z'])
    >>> df.select(dbf.st_astext(dbf.st_makepoint('x', 'y', 'z')).alias('result')).collect()
    [Row(result='POINT Z (10 34 1)')]

    Example 3: Create a 4D point.
    >>> from pyspark.databricks.sql import functions as dbf
    >>> df = spark.createDataFrame([(10.0, 34.0, 1.0, 2.0)], ['x', 'y', 'z', 'm'])
    >>> df.select(dbf.st_astext(dbf.st_makepoint('x', 'y', 'z', 'm')).alias('result')).collect()
    [Row(result='POINT ZM (10 34 1 2)')]
    """
    if len(cols) not in (2, 3, 4):
        raise PySparkValueError(
            errorClass="WRONG_NUM_COLUMNS_RANGE",
            messageParameters={
                "func_name": "st_makepoint",
                "min_num_cols": "2",
                "max_num_cols": "4",
                "num_cols": str(len(cols)),
            },
        )
    sc = SparkContext._active_spark_context
    assert sc is not None and sc._jvm is not None
    java_cols = [to_java_column(c) for c in cols]
    jc = sc._jvm.com.databricks.sql.functions.st_makepoint(*java_cols)
    return Column(jc)
...
@try_remote_edge_functions
def st_nrings(col: "ColumnOrName") -> Column:
    """Returns the number of rings in the input polygon or multipolygon GEOGRAPHY or GEOMETRY.
    ...
    ----------
    col : :class:`~pyspark.sql.Column` or str
        A GEOGRAPHY or GEOMETRY value representing a polygon or multipolygon.
    ...
    --------
    >>> from pyspark.databricks.sql import functions as dbf
    >>> df = spark.createDataFrame([('POLYGON EMPTY','POLYGON ((0 0,10 0,0 10,0 0))')], ['pgn1','pgn2'])
    >>> df.select(dbf.st_nrings(dbf.st_geomfromtext('pgn1')).alias('result')).collect()
    [Row(result=0)]
    >>> df = spark.createDataFrame([('POLYGON ((0 0,50 0,50 50,0 50,0 0),(20 20,25 30,30 20,20 20))',)], ['pgn'])
    >>> df.select(dbf.st_nrings(dbf.st_geomfromtext('pgn')).alias('result')).collect()
    [Row(result=2)]
    >>> df = spark.createDataFrame([('MULTIPOLYGON (((0 0,10 0,10 10,0 10,0 0),(1 1,4 1,4 4,1 4,1 1),(5 5,6 5,6 6,5 6,5 5)),((20 20,30 20,30 30,20 30,20 20)))',)], ['mpgn'])
    >>> df.select(dbf.st_nrings(dbf.st_geogfromtext('mpgn')).alias('result')).collect()
    [Row(result=4)]
    """
    sc = SparkContext._active_spark_context
    assert sc is not None and sc._jvm is not None
    jc = sc._jvm.com.databricks.sql.functions.st_nrings(to_java_column(col))
    return Column(jc)
...
def st_setsrid(col1: "ColumnOrName", col2: Union["ColumnOrName", int]) -> Column:
    ...
    [Row(result=4326)]
    """
    return ossf.st_setsrid(col1, col2)
