...
def _convert_exception(
    ...
    # BEGIN-EDGE
    # [Somewhat ugly hack]
    # If sql_state is not provided, try to look it up via error_class, same way as
    # PysparkException does.
    # SparkConnectGrpcException.getSqlState() does this lookup by default, but in the EDGE
    # code we replace empty sql_state with either
    # CLIENT_UNEXPECTED_MISSING_SQL_STATE or CLIENT_INTERNAL_ERROR before creating a
    # SparkConnectGrpcException instance. For this reason we need to do the lookup in
    # _convert_exception before creating the exception as well.
    # If the sql_state is not provided and cannot be looked up by error_class, we set it to
    # CLIENT_UNEXPECTED_MISSING_SQL_STATE.
    if sql_state is None:
        error_reader = ErrorClassesReader()
        sql_state = error_reader.get_sqlstate(error_class)
    if sql_state is None:
