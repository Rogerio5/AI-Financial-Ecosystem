...
class ErrorClassesReader:
    ...
    def get_error_message(self, errorClass: str, messageParameters: Dict[str, str]) -> str:
        ...
        # BEGIN-EDGE: changed == to <= to allow extra params from unknown subclass fallback
    ...
    def get_message_template(self, errorClass: str) -> str:
        ...
        if not has_sub_class:
            ...
            # BEGIN-EDGE
            # Gracefully handle the case where the server returns a subclass that the client
            # doesn't know about. This can happen when a newer server returns error subclasses
            # to an older client that doesn't have the updated error class definitions.
            # In this case, we fall back to using just the main error class message template.
            if "sub_class" not in main_error_class_info_map:
                # Server sent a subclass but client doesn't have any subclasses for this error
                message_template = main_message_template
            elif sub_error_class not in main_error_class_info_map["sub_class"]:
                # Server sent a subclass that client doesn't recognize
                message_template = main_message_template
            else:
            # END-EDGE
    ...
    def get_breaking_change_info(self, errorClass: Optional[str]) -> Optional[Dict[str, Any]]:
        ...
        if len_error_classes == 2:
            sub_error_class = error_classes[1]
            # BEGIN-EDGE
            # Gracefully handle unknown subclasses - fall back to main class breaking change info
            if "sub_class" not in main_error_class_info_map:
                pass  # Fall through to main class breaking change info
            elif sub_error_class not in main_error_class_info_map["sub_class"]:
                pass  # Fall through to main class breaking change info
            else:
            # END-EDGE
...
def _capture_call_site(depth: int) -> str:
    ...
    if ipython and hasattr(ipython, 'parent_header'):
