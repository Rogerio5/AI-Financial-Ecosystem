...
from pyspark.sql.udf_logger import PySparkUDFLogger # EDGE
...
if IS_ENGINE_PYSPARK:
    from pyspark._engine_adapters import (
        convert_return_type,
        engine_pyspark_function_converter,
        isinstance,
        type,
        client_pyspark_taskcontext,
        get_client_barrier_task_context_from_json,
        get_client_task_context_from_json,
    )
    ClientTaskContext = client_pyspark_taskcontext.TaskContext
# END-EDGE
...
class EvalConf(Conf):
    ...
    # BEGIN-EDGE
    @property
    def is_isolated_tws_enabled(self) -> bool:
        return self.get("is_isolated_tws_enabled", "false") == "true"

    @property
    def state_server_id(self) -> Optional[Union[int, str]]:
        sid = self.get("state_server_id", None)
        try:
            return int(sid)
        except ValueError:
            return sid
...
def read_udfs(pickleSer, infile, eval_type, runner_conf, eval_conf):
    if eval_type in (
        ...
        if (
            ...
            # Edge deserializer supporting slicing
        ...
        elif eval_type == PythonEvalType.SQL_COGROUPED_MAP_PANDAS_UDF and should_use_edge:
            ...
            batch_timeout_ms = runner_conf.get(
    ...
    if eval_type == PythonEvalType.SQL_GROUPED_MAP_PANDAS_UDF:
        ...
        def mapper(batch_iter):
            ...
            # The JVM side (writeNextInputToStream) guarantees at least one
            # batch per group via assertion: 0 < numRowsInBatch.
            table = pa.Table.from_batches(list(batch_iter)).combine_chunks()
        ...
        stateful_processor_api_client = StatefulProcessorApiClient(
            eval_conf.state_server_id,
            eval_conf.grouping_key_schema,
            is_driver=False,
            is_isolated_tws_enabled=eval_conf.is_isolated_tws_enabled,
        )
    ...
    # BEGIN-EDGE
        ...
        # BEGIN-EDGE
        worker_cache_enabled_conf_key = (
            "spark.databricks.streaming.realTimeMode.transformWithState.workerCache.enabled"
        )
        worker_cache_enabled = runner_conf.get(
            worker_cache_enabled_conf_key, "false"
        ).lower() == "true"
        # END-EDGE
        ...
        stateful_processor_api_client = StatefulProcessorApiClient(
            ...
            worker_cache_enabled=worker_cache_enabled, # EDGE
        )
...
def main(infile, outfile):
    try:
        ...
        if task_context_json["isBarrier"]:
            if IS_ENGINE_PYSPARK:
                taskContext = get_client_barrier_task_context_from_json(task_context_json)
            else:
                taskContext = BarrierTaskContext.from_json(task_context_json)
        else:
            if IS_ENGINE_PYSPARK:
                taskContext = get_client_task_context_from_json(task_context_json)
            else:
        ...
        if IS_ENGINE_PYSPARK:
            # Ensure TaskContext is set for the client TaskContext.
            ClientTaskContext._setTaskContext(taskContext)
        else:
