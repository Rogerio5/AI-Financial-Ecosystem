...
def _start_update_server(
    ...
    if is_unix_domain_sock:
        ...
        # BEGIN-EDGE
        # if repl is sandboxed, it will be in separate network namespace from driver and has its own IP address
        def _get_host_ip():
            """
            Get container eth0 ip address, example:
              'eth0': [
                snicaddr(family=<AddressFamily.AF_INET: 2>, address='10.10.145.236', netmask='255.255.224.0', broadcast='10.10.159.255', ptp=None),
                snicaddr(family=<AddressFamily.AF_PACKET: 17>, address='02:0b:bf:5f:bf:ed', netmask=None, broadcast='ff:ff:ff:ff:ff:ff', ptp=None)
              ]
            """
            import psutil
            import socket
        ...
        if os.getenv("IS_SANDBOXED_REPL", "false") == "true" or os.getenv("IS_DLT_SANDBOX", "false") == "true":
            host_ip = _get_host_ip()
        # END-EDGE

        server = AccumulatorTCPServer((host_ip, 0), UpdateRequestHandler, auth_token)
