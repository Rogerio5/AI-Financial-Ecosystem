...
class RemoteMLWriter(MLWriter):
    ...
    def handleOverwrite(path: str, shouldOverwrite: bool) -> None:
        ...
        if shouldOverwrite:
            ...
            if parsed_url.scheme in ['', 'dbfs'] and parsed_url.path.startswith('/Volumes'):
                # If this is a UC volume path, Hadoop FS deletion API does not work,
                # but in Databricks runtime, UC volume path is mounted as local FS path,
                # so it can be deleted through local FS deletion API.
                if os.path.exists(parsed_url.path):
